  ...
  doRadio() {
    ...
    alert.addButton({
      ...
      handler: data => {
        ...
        console.log(this.amounttest);

        for (var i=0; i<this.amounttest.length; i++)
        {
          console.log(this.amounttest.charAt(i));
        ...
        if(this.interfaceid!=null && this.name!=null && this.amount!=null && amountvalidate )
        {
          ...
          //if(this.interfaceid==this.data.id && this.name==this.data.firstname) {
            this.http.get('http://172.22.169.251/CPMAD/Shuttle/payment.php?id=' + this.id + '&amount=' + this.amount).timeout(20000)
                .map(res => res.json())
